...
public class SKAdmin {
  ...
  public SKAdmin(SKGridConfiguration gc, SKAdminOptions options) throws IOException, KeeperException {
    ...
    if (ringConfigAndTree == null) {
      ...
      cloudMC = new com.ms.silverking.cloud.meta.MetaClient(ringConfig.getCloudConfiguration(),
          dhtMC.getZooKeeper().getZKConfig());
    ...
    if (options.defaultClassVars != null) {
      defaultClassVars = DHTConstants.defaultDefaultClassVars.overrideWith(
          classVarsZK.getClassVars(options.defaultClassVars));
    } else {
      if (dhtConfig.getDefaultClassVars() != null) {
        defaultClassVars = DHTConstants.defaultDefaultClassVars.overrideWith(
            classVarsZK.getClassVars(dhtConfig.getDefaultClassVars()));
      } else {
    ...
    commonNSOptions = "revisionMode=NO_REVISIONS,storageType=FILE,storageFormat=" + options.storageFormat + "," +
    ...
    skfsFileBlockNSOptions = NamespaceOptions.parse(
        "versionMode=SYSTEM_TIME_NANOS," + commonNSOptions + "," + opOptions + fileBlockNSValueRetentionPolicy);
    System.out.println(skfsFileBlockNSOptions);
    skfsDirNSOptions = NamespaceOptions.parse(
        "versionMode=SYSTEM_TIME_NANOS," + commonNSOptions + "," + dirNSOpOptions + "," + dirNSValueRetentionPolicy + dirNSSSOptions);
  ...
  public static Pair<RingConfiguration, InstantiatedRingTree> getRing(DHTConfiguration dhtConfig,
    ...
    ringMC = com.ms.silverking.cloud.toporing.meta.MetaClient.createMetaClient(ringName, ringVersion.getV1(),
        dhtMC.getZooKeeper().getZKConfig());
    ...
    /*
    private static Map<String,HostGroupTable> getHostGroupTables(Set<String> hostGroupNames, ZooKeeperConfig
    zkConfig) throws KeeperException, IOException {
  ...
  private static HostGroupTable getHostGroupTable(String hostGroupTableName, ZooKeeperConfig zkConfig)
    ...
    cloudMC = new com.ms.silverking.cloud.meta.MetaClient(
  ...
  private String getDHTOptions(SKAdminOptions options, ClassVars classVars) {
    return getSystemPropertyFormatted("com.ms.silverking.Log", options.logLevel) + getSystemPropertyFormatted(
        DHTConstants.dataBasePathProperty, getDataDir(classVars)) + getSystemPropertyFormatted(
        DHTConstants.fileSegmentCacheCapacityProperty,
        getFileSegmentCacheCapacity(classVars)) + getSystemPropertyFormatted(
        DHTConstants.retrievalImplementationProperty,
        getRetrievalImplementation(classVars)) + getSystemPropertyFormatted(DHTConstants.segmentIndexLocationProperty,
        getSegmentIndexLocation(classVars)) + getSystemPropertyFormatted(DHTConstants.nsPrereadGBProperty,
        getNSPrereadGB(classVars));
  }
  ...
  private String getExtraOptions(SKAdminOptions options, ClassVars classVars, boolean escaped) {
    ...
    if (classVars.getVarMap().containsKey(BaseDirectoryInMemorySS.compressionProperty)) {
      s += getSystemPropertyFormatted(BaseDirectoryInMemorySS.compressionProperty,
  ...
  private String createStartCommand(DHTConfiguration dhtConfig, ClassVars classVars, SKAdminOptions options) {
    ...
    return "echo \"" + _createStartCommand(dhtConfig, classVars,
        options) + "\" > " + cmdFile + "; chmod +x " + cmdFile + "; nohup " + cmdFile + " 1> /dev/null 2>&1 < " +
        "/dev/null &";
  }
  ...
  private String _createStartCommand(DHTConfiguration dhtConfig, ClassVars classVars, SKAdminOptions options) {
    ...
    return (destructive ? "" : "netstat -tulpn | grep tcp.*:" + dhtConfig.getPort() + " ; ") + (destructive ?
        "" :
        "if [ \\$? -ne 0 ]; then { ") + (destructive ?
        createStopCommand(dhtConfig, classVars) + "; " :
        "") + "mkdir -p " + daemonLogDir + "; " + "rm " + getHeapDumpFile(
        classVars) + "; " + "mv " + daemonLogFile + " " + prevDaemonLogFile + "; " + getPreJavaCommand(
        classVars) + " " + getNodeEnv(classVars) + getTaskset(options) + _generateNodeStartCommand(classVars, options,
        reapPolicy, true) + (destructive ?
        (" 1>" + daemonLogFile + " 2>&1 &") :
  ...
  public String generateSingleNodeStartCommand(SKAdminOptions options) throws IOException, KeeperException {
    ...
    if (targetServers.size() > 1) {
      throw new IllegalArgumentException(
          "generation of single node start command should only be passed a single target, got " + targetServers.toString());
    ...
    serverClassVars = getServerClassVars(thisServer, hostGroupTable, activeHostGroupNames, passiveNodeHostGroupNames,
  ...
  private String _generateNodeStartCommand(ClassVars classVars, SKAdminOptions options, ReapPolicy reapPolicy,
      boolean escaped) {
  ...
  private String createCheckSKFSCommand(DHTConfiguration dhtConfig, ClassVars classVars) {
    ...
    return "mkdir -p " + logDir + "; " + getCheckSKFSBaseCommand(classVars,
        "CheckSKFS") + " -C " + options.compression + " -l " + options.logLevel + (options.coreLimit == null ?
        "" :
        " -L " + options.coreLimit) + (options.checkSKFSOptions == null ? "" : " " + translateOptionEncoding(
        options.checkSKFSOptions))
  ...
  private String createStopSKFSCommand(DHTConfiguration dhtConfig, ClassVars classVars) {
    ...
    return "mkdir -p " + logDir + "; " + getCheckSKFSBaseCommand(classVars, "StopSKFS") + " 1>" + logFile + " 2>&1 &";
  }
  ...
  private String getCheckSKFSBaseCommand(ClassVars classVars, String command) {
    return (skGlobalCodebase == null ?
        "" :
        "export skGlobalCodebase=" + skGlobalCodebase + "; ") + "export " + GridConfiguration.defaultBaseEnvVar + "=" + (
        options.gridConfigBase == null ?
            GridConfiguration.getDefaultBase() :
            new File(
                options.gridConfigBase).getAbsolutePath()) + "; " + "export " + DHTConstants.jaceHomeEnv + "=" + PropertiesHelper.envHelper.getString(
        DHTConstants.jaceHomeEnv,
        UndefinedAction.ExceptionOnUndefined) + "; " + "export " + DHTConstants.javaHomeEnv + "=" + PropertiesHelper.envHelper.getString(
        DHTConstants.javaHomeEnv,
        getSystemJavaHome()) + "; " + "export " + DHTConstants.classpathEnv + "=" + PropertiesHelper.envHelper.getString(
        DHTConstants.classpathEnv, System.getProperty(DHTConstants.classpathProperty)) + "; " + getTaskset(
        options) + classVars.getVarMap().get(
        DHTConstants.checkSKFSCommandVar) + " -g " + gc.getName() + " -z " + gc.getClientDHTConfiguration().getZKConfig() + " -c " + command + " -f " + options.forceSKFSDirectoryCreation + skfsTimeoutOption(
        "skfsEntryTimeoutSecs", options.skfsEntryTimeoutSecs) + skfsTimeoutOption("skfsAttrTimeoutSecs",
        options.skfsAttrTimeoutSecs) + skfsTimeoutOption("skfsNegativeTimeoutSecs", options.skfsNegativeTimeoutSecs);
  }
  ...
  private String skfsTimeoutOption(String name, int timeout) {
    if (timeout == SKAdminOptions.skfsTimeoutNotSet) {
      ...
      switch (name) {
      case "skfsEntryTimeoutSecs":
        ch = "E";
        break;
      case "skfsAttrTimeoutSecs":
        ch = "A";
        break;
      case "skfsNegativeTimeoutSecs":
        ch = "N";
        break;
      default:
  ...
  private boolean execAdminCommandGroup(SKAdminCommand[] commands)
  ...
  private boolean setInstanceExclusions() throws KeeperException, IOException {
    return setInstanceExclusions(options.targets != null ?
        ExclusionSet.parse(options.targets) :
        ExclusionSet.emptyExclusionSet(VersionedDefinition.NO_VERSION));
  }
  ...
  private void changeInstanceExclusions(Set<IPAndPort> serversToChange, SKAdminCommand command)
      throws KeeperException, IOException {
  ...
  private boolean addToInstanceExclusions() throws KeeperException, IOException {
    if (options.targets != null) {
      changeInstanceExclusions(
          ExclusionSet.parse(options.targets).asIPAndPortSet(gc.getClientDHTConfiguration().getPort()),
          SKAdminCommand.AddToInstanceExclusions);
    }
    return true;
  }
  ...
  private boolean removeFromInstanceExclusions() throws KeeperException, IOException {
    if (options.targets != null) {
      changeInstanceExclusions(
          ExclusionSet.parse(options.targets).asIPAndPortSet(gc.getClientDHTConfiguration().getPort()),
          SKAdminCommand.RemoveFromInstanceExclusions);
    }
    return true;
  }
  ...
  private Set<String> retainOnlySpecifiedAndNonExcludedServers(Set<String> servers, Set<String> _targetServers,
      HostGroupTable hostGroupTable) {
    Set<String> _servers;
    Set<String> targetServers;
  ...
  private void verifyServerEligibility(Set<String> servers, SKAdminCommand[] commands) throws KeeperException {
    if (Arrays.contains(commands, SKAdminCommand.StartNodes)) {
      ...
      ineligibleServers = HealthMonitor.removeIneligibleServers(candidateServers, dhtRingCurTargetZK,
          instanceExclusionZK);
      if (ineligibleServers.size() > 0) {
        ...
        if (!options.forceInclusionOfUnsafeExcludedServers) {
          throw new IneligibleServerException(
              "Attempted to start ineligible servers: " + CollectionUtil.toString(ineligibleServers, ','));
        } else {
          Log.countdownWarning("*** Including unsafe excluded servers. This may result in data loss ***",
              options.unsafeWarningCountdownSecs);
        }
      } else {
  ...
  public InstantiatedRingTree readCurrentTree() throws KeeperException, IOException {
    ...
    ringTree = SingleRingZK.readTree(new com.ms.silverking.cloud.toporing.meta.MetaClient(
            new NamedRingConfiguration(dhtConfig.getRingName(), ringConfig), dhtMC.getZooKeeper().getZKConfig()),
        curRingAndVersionPair.getTail());
    return ringTree;
  }
  ...
  private boolean execClusterCommandGroup(SKAdminCommand[] commands) throws IOException, KeeperException {
    ...
    for (SKAdminCommand command : commands) {
      ...
      serverCommands = createServerCommands(command, validActiveServers, validPassiveServers, hostGroupTable,
          hostGroupToClassVars, activeHostGroupNames, passiveNodeHostGroupNames);
      displayCommandMap(serverCommands);
      if (!options.displayOnly) {
        _result = execCommandMap(serverCommands,
            validActiveServers.size() > 0 ? validActiveServers : validPassiveServers, hostGroupTable);
        result = result && _result;
        if (!result) {
          break;
        }
  ...
  private Map<String, ClassVars> getHostGroupToClassVarsMap(DHTConfiguration dhtConfig) throws KeeperException {
    ...
    for (Map.Entry<String, String> hostGroupAndClassVarsName : hostGroupToClassVarNames.entrySet()) {
      hostGroupToClassVars.put(hostGroupAndClassVarsName.getKey(),
          classVarsZK.getClassVars(hostGroupAndClassVarsName.getValue()));
    }
    ...
    case ZooKeeper:
      try {
        ...
        throw new RuntimeException(
            "Execution of ClearData fails: cannot clear namespace options data in mode [" + mode + "]", e);
      }
      break;
    case MetaNamespace:
  ...
  private ClassVars getServerClassVars(String server, HostGroupTable hostGroupTable, Set<String> activeHostGroupNames,
      Set<String> passiveNodeHostGroupNames, Map<String, ClassVars> hostGroupToClassVars) {
  ...
  /**
   * The exposed function to run SKAdmin's real business logic, this method is the entry point for user who wishes
   * to use SKAdmin as library
   *
  ...
  public static void runWithParsedOptions(SKAdminOptions options, SKGridConfiguration skConfigOverride) {
    ...
    try {
      ...
      if (options.forceInclusionOfUnsafeExcludedServers) {
        Log.countdownWarning("Options requesting unsafe excluded servers. This may result in data loss",
            options.unsafeWarningCountdownSecs);
      }
